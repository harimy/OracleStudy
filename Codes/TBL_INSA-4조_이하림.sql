...
SELECT BUSEO "부서", 서DECODE(SUBSTR(SSN, 8, 1), '1', '남자', '2', '여자') "성별", COUNT(*) "인원수"
FROM TBL_INSA
...
SELECT CITY "지역", DECODE(SUBSTR(SSN, 8, 1), '1', '남자', '2', '여자') "성별", COUNT(*) "인원수"
FROM TBL_INSA
...
SELECT EXTRACT(YEAR FROM IBSADATE) "입사년도", DECODE(SUBSTR(SSN, 8, 1), '1', '남', '2', '여') "성별", COUNT(*) "인원수"
FROM TBL_INSA 
...
SELECT BUSEO "부서", DECODE(SUBSTR(SSN, 8, 1), '1', '남', '2', '여') "성별", COUNT(*) "인원수", SUM(BASICPAY) "급여합"
FROM TBL_INSA
WHERE CITY = '서울' 
GROUP BY BUSEO, SUBSTR(SSN, 8, 1)
ORDER BY 1;
...
SELECT BUSEO, DECODE(SUBSTR(SSN, 8, 1), '1', '남', '2', '여') "성별", COUNT(*)
FROM TBL_INSA
GROUP BY BUSEO, SUBSTR(SSN, 8, 1)
HAVING COUNT(*) >= 4
ORDER BY 1;
...
SELECT BUSEO "부서", JIKWI "직위", COUNT(*) "인원수", SUM(BASICPAY) "급여합", ROUND(AVG(BASICPAY)) "급여평균"
FROM TBL_INSA
GROUP BY BUSEO, JIKWI
ORDER BY 1;
--==>>
...
-- 이문제 모르겠다
-- 주민번호는 이미 CHAR 타입인데 TO_CHAR를 어디다 쓰라는 것...?  ㅜ 
-- SUBSTR(SSN, 3, 2) = TO_CHAR(05)
-- 이런건가 ㅜ? 
/*
24. 주민번호 기준으로 5월 생만 조회. 
    단, TO_CHAR() 함수 이용.
...
-- DECODE() 쓰는거 잘 모르겠다. 
-- 38번 나머지 부서들 안나오게 하고싶은데 어떻게 해야하지?
-- 타부서를 안넣어도 null 값으로 나와서,, 
-- NULL 값은 제외하고 선택하고 싶은데 ... 
--> IS NOT NULL 썼다. 
/*
SELECT DECODE(BUSEO, '개발부', '개발부', '영업부', '영업부', '총무부', '총무부', '타부서') "부서" , COUNT(*) "인원수"
FROM TBL_INSA
GROUP BY DECODE(BUSEO, '개발부', '개발부', '영업부', '영업부', '총무부', '총무부', '타부서')
ORDER BY 1;
*/
--> WHERE ~ IS NOT NULL 로 해결. 

-- 여자 인원수가 5명이상인 조건에서 남여 인원수를 어떻게 조회해야 할 지 모르겠다.
/*
51. 부서별 남,여 인원수 출력. 여자인원수가 5명 이상인 부서만 조회.
SELECT BUSEO, DECODE(SUBSTR(SSN, 8, 1), '1', '남', '2', '여') "성별", COUNT(*)
FROM TBL_INSA
GROUP BY BUSEO, SUBSTR(SSN, 8, 1);
HAVING COUNT(SUBSTR(SSN, 8, 1) = '2') > 5;
*/
